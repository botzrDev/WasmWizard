...
fn test_wasm_modules_exist_and_valid() {
    ...
    for module in &modules {
        ...
        assert_eq!(&content[0..4], &[0x00, 0x61, 0x73, 0x6D], "WASM magic bytes should be correct for {}", module);
...
fn test_wasm_test_data_exists() {
    ...
    for (module, input_file, output_file) in &test_cases {
        ...
        let input_content = fs::read_to_string(&input_path).expect("Should be able to read input file");
        let expected_output = fs::read_to_string(&output_path).expect("Should be able to read output file");
        
        println!("✓ {} test case: input='{}' expected_output='{}'", 
                module, 
                input_content.trim(), 
...
fn test_file_size_validation() {
    ...
    // Test input files
    let inputs = ["calc_add_input.txt", "echo_input.txt", "hello_world_input.txt"];
    for input in &inputs {
...
fn test_security_validation() {
    ...
    for pattern in &malicious_patterns {
        ...
        let is_suspicious = malicious_patterns.iter()
    ...
    let is_safe = !malicious_patterns.iter()
...
fn test_api_key_format() {
    ...
    for key in &invalid_keys {
        assert!(!key.starts_with("ww_") || key.len() <= 3, 
...
fn test_rate_limiting_logic() {
    ...
    for (tier_name, per_minute, per_day) in &tiers {
        ...
        assert!(*per_day >= *per_minute, "Daily limit should be >= minute limit for {}", tier_name);
        ...
        assert!(*per_day <= theoretical_max_per_day, 
               "Daily limit should be achievable for {}: {} <= {}", 
               tier_name, per_day, theoretical_max_per_day);
        
        println!("✓ {} tier limits: {}/min, {}/day (ratio: {:.2})", 
                tier_name, per_minute, per_day, 
...
fn test_error_message_sanitization() {
    ...
    for error in &sensitive_errors {
        ...
        assert!(!sanitized.contains("/etc/"), "Sanitized error should not contain sensitive paths");
        ...
        for value in values {
            ...
            match *config_name {
                "MAX_WASM_SIZE" => {
                    ...
                    println!("✓ {}: {} -> {}", config_name, value, if is_valid { "valid" } else { "invalid" });
                },
                "MAX_INPUT_SIZE" => {
