...
async fn setup_test_environment() -> PgPool {
    ...
    let port = postgres_node.get_host_port(5432).expect("Failed to get postgres port");
    ...
    let pool = establish_connection_pool(&config).await.expect("Failed to establish test database connection");
...
async fn test_wasm_execution_with_valid_auth() {
    ...
    let body: Value = test::read_body_json(resp).await;
    assert!(body["error"].as_str().unwrap().contains("Invalid WASM file format"));
}
...
async fn test_wasm_invalid_module_handling() {
    ...
    let body: Value = test::read_body_json(resp).await;
    assert!(body["error"].as_str().unwrap().contains("Missing 'wasm' field"));
}
...
async fn test_invalid_tier_handling() {
    ...
    let body: Value = test::read_body_json(resp).await;
    assert!(body["error"].as_str().unwrap().contains("Invalid tier name"));
}
...
async fn test_subscription_tiers_exist() {
    ...
    let tiers: Vec<(String,)> = sqlx::query_as(
        "SELECT name FROM subscription_tiers ORDER BY name"
    )
    .fetch_all(&pool)
    .await
