...
impl RedisRateLimiter {
    ...
    pub async fn get_remaining_requests(
        ...
        // Calculate remaining
        let remaining_minute = rate_limit.requests_per_minute.saturating_sub(minute_count as u32);
...
mod tests {
    ...
    async fn test_redis_rate_limiter() {
        ...
        let _: () = redis::cmd("DEL").arg(&minute_key).arg(&day_key).query_async(&mut conn).await.unwrap();
        
        // Test initial state
        let (remaining_minute, remaining_day) = rate_limiter.get_remaining_requests(api_key_id, &rate_limit).await.unwrap();
        assert_eq!(remaining_minute, 5);
        assert_eq!(remaining_day, 10);
        
        // Test minute limit
        for i in 0..5 {
            let (allowed, _) = rate_limiter.check_rate_limit(api_key_id, &rate_limit).await.unwrap();
            assert!(allowed, "Request {} should be allowed", i+1);
        }
        
        // The 6th request should be denied
        let (allowed, retry_after) = rate_limiter.check_rate_limit(api_key_id, &rate_limit).await.unwrap();
